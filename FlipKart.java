...
public class FlipKart {
	...
	public static void main(String[] args) throws IOException {
		...
		//InputStream inputStream = registrationPage.class.getResourceAsStream("config.properties");
		//prop.load(inputStream);
		
		try(InputStream inputStream = FlipKart.class.getResourceAsStream("config.properties")) {
			prop.load(inputStream);
	    }
	    catch(Exception e) {
		...
		//Logging in
		driver.findElement(By.xpath("//input[@type='text' and @class='_2zrpKA']")).sendKeys(prop.getProperty("USER"));
		...
		FileHandler.copy(SrcFile,new File(prop.getProperty("screenshotPath")));
		timeDelay(500);
		//Remove from cart
